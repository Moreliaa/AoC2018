...
public class Day1 {
	public static void main(String[] args) {
		...
		while (true) {
			...
			if (operators.get(index) == '+') {
				acc += frequencyChanges.get(index);
			} else {
				acc -= frequencyChanges.get(index);
			}
			index++;
	...
	private static void parseInput(String input, ArrayList<Character> operators, ArrayList<Integer> frequencyChanges) {
		...
		for (int i = 0; i < input.length(); i++) {
			...
			if (c == '+' || c == '-') {
				operators.add(c);
				if (currentNumber != "") {
					frequencyChanges.add(Integer.valueOf(currentNumber));
					currentNumber = "";
				}
			} else {
				currentNumber = currentNumber.concat(c.toString());
			}
		}
		frequencyChanges.add(Integer.valueOf(currentNumber));
	}
